...
@objc(FIRStorage) open class Storage: NSObject {
  ...
  @objc(storageForApp:) open class func storage(app: FirebaseApp) -> Storage {
      guard let provider = ComponentType<StorageProvider>.instance(for: StorageProvider.self,
          ...
          fatalError("StorageProvider not found in app container")
      }
  ...
  internal init(app: FirebaseApp, bucket: String) {
    self.app = app
      guard let auth = ComponentType<AuthInterop>.instance(for: AuthInterop.self,
                                                               in: app.container) else {
              fatalError("AuthInterop not found in app container")
          }
          self.auth = auth
      self.appCheck = ComponentType<AppCheckInterop>.instance(for: AppCheckInterop.self,
                                                              in: app.container)
  ...
  private static func initFetcherServiceForApp(_ app: FirebaseApp,
                                               _ bucket: String,
                                               _ auth: AuthInterop,
                                               _ appCheck: (any AppCheckInterop)? )
